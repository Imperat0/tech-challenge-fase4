...
def executar_pipeline(video, texto_clinico, id_paciente):
    ...
    try:
        ...
        if video is None:
            ...
            erro_html = (
                "<div style='color: #ef4444; padding: 10px; border: 1px solid #ef4444; "
                "border-radius: 8px;'>⚠️ Por favor, selecione um vídeo de teste.</div>"
            )
            return erro_html, "", {"status": "erro", "detalhe": "Vídeo não fornecido"}

        logger.info(
            f"📹 Vídeo recebido: tipo={type(video)}, tamanho={len(video) if hasattr(video, '__len__') else 'unknown'}"
        )

        # Transforma string vazia ou espaços em None para ativar o fallback do Whisper
        texto_filtrado = (
            texto_clinico.strip()
            if texto_clinico and texto_clinico.strip()
            else None
        )
        id_filtrado = (
            id_paciente.strip()
...
""".strip()
        ...
        return (
            ...
            resultado
            if isinstance(resultado, dict)
            else {"resultado_bruto": str(resultado)},
        )
        ...
        erro_html = (
            "<div style='color: #ef4444; padding: 10px; border: 1px solid #ef4444; "
            "border-radius: 8px;'>❌ Arquivo não encontrado:<br>"
            f"{error_msg}<br><br>"
            "📍 Verifique se o vídeo foi carregado corretamente.</div>"
        )
        ...
        erro_html = (
            "<div style='color: #ef4444; padding: 10px; border: 1px solid #ef4444; "
            "border-radius: 8px;'>❌ Erro de dependência:<br>"
            f"{error_msg}<br><br>"
            "🔧 Dependências disponíveis:<br>"
            f"{deps_html}</div>"
        )
        ...
        erro_html = (
            "<div style='color: #ef4444; padding: 10px; border: 1px solid #ef4444; "
            "border-radius: 8px;'>❌ Valor inválido nos parâmetros:<br>"
            f"{error_msg}</div>"
        )
        ...
        erro_html = (
            "<div style='color: #ef4444; padding: 10px; border: 1px solid #ef4444; "
            "border-radius: 8px;'>❌ Erro durante execução (API/Modelo):<br>"
            f"{error_msg}<br><br>"
            "💡 Possível causa: API Key faltando ou limite de rate limit atingido.</div>"
        )
        ...
        erro_html = (
            "<div style='color: #ef4444; padding: 10px; border: 1px solid #ef4444; "
            "border-radius: 8px;'>❌ Erro interno durante a execução do pipeline:<br>"
            f"{error_msg}<br><br>"
            "📋 Verifique os logs do terminal para o stack trace completo.</div>"
        )
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            input_text = gr.Textbox(
                label="Laudo / Texto Clínico (Opcional)",
                placeholder="Ex: Paciente gestante, 32 semanas... (Deixe em branco para usar o áudio do vídeo)",
            )
